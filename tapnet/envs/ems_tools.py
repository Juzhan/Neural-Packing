...
def compute_ems(id_maps, heightmaps, container_size, min_ems_width=0, ems_type='ems-id-stair', valid_container_mask=None):
    ...
    for container_id in range(len(heightmaps)):
        ...
        heightmap = heightmaps[container_id]
...
def compute_box_ems_mask(box_states, ems, box_num=None, all_container_space=None, heightmaps=None, packing_mask=None, remove_list=None, check_box_stable=True, gripper_size=None, check_z=True):
    # can place or not
        ...
        for pair in valid_pair:
            ...
            if check_box_stable and in_packing_mask:
                if len(all_container_space) > 0:
                    ...
                    is_stable = space.drop_box_virtual([bx,by,bz], [lx, ly], False, 1, 1)
