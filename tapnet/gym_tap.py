...
class TAPer:
    ...
    def init_packing_state(self, pre_boxes=None, heightmap=None, packing_mask=None, reset=False):
        ...
        if pre_boxes is not None:
            ...
            for pre_b in pre_boxes:
                ...
                pos = np.array([x,y,z]) * self.unit_scale
    ...
    def pick_one_to_pack(self, boxes, check_box_stable=True):
        ...
        probs, _state = self.actor( OBS(obs) )
        ...
        return False, box, pos, idx, hm, container_id
    ...
    def pack_box(self, boxes, check_box_stable=True, allow_preview=True, invert=False):
        ...
        if len(boxes) == 0:
            return False, ret_box, ret_id, container_id

        if allow_preview: 
            done, box, pos, idx, hm, container_id = self.pick_one_to_pack(boxes, check_box_stable)
            if done:
